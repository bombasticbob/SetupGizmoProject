...
#define _T(X) X /* for now just do this */
...
void CLoginDlg::UpdateData(BOOL bSaveFlag)
{
  if(bSaveFlag)
  {
    GetDlgItemText(IDC_USERNAME, m_csUserName);
    GetDlgItemText(IDC_PASSWORD, m_csPassword);
    GetDlgItemText(IDC_DOMAIN, m_csDomain);
  }
  else
  {
    SetDlgItemText(IDC_USERNAME, m_csUserName);
    SetDlgItemText(IDC_PASSWORD, m_csPassword);
    SetDlgItemText(IDC_DOMAIN, m_csDomain);
  }
}
